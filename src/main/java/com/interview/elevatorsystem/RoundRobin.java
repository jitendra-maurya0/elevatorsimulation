...
public class RoundRobin {
     ...
     *
     * The goal is to achieve an equal load for all lift.
     *
     ...
     */
    public int choseElevator(Elevator[] elevatorGroup, int noOfElevators) {

        int pick = (++this.counter) % noOfElevators;
        ...
        while(elevatorGroup[pick].getCapacity() < (elevatorGroup[pick].getSequence().size() + 1)) {
            pick = (++this.counter) % noOfElevators;
        }
